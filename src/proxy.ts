...
/**
 * Next 16 file-convention proxy (formerly `middleware.ts`). Refreshes the
 * Supabase auth cookie on page navigations when the user has a session,
 * short-circuits for guests via the `sb-*` cookie sniff.
 */
export async function proxy(request: NextRequest) {
  ...
  // proxy to read different cookie keys than the callback set.
...
export const config = {
  matcher: [
    // Static assets, APIs, SEO — skip the proxy. Page navigations + auth routes still run.
