...
import { type HancockCountyFairSignupSheetRow } from "@/lib/hancock-county-fair-signups";
...
export function HancockCountyFairSignupTable({
  rows,
  addSignup,
  signupsEnabled = true,
}: {
  rows: HancockCountyFairSignupSheetRow[];
  ...
  return (
    <div className="md:overflow-x-auto">
      <table
        aria-label="Hancock County Fair 2026 signup sheet"
      ...
      >
        <caption className="sr-only">Hancock County Fair 2026 signup sheet</caption>
        ...
        <tbody className="block md:table-row-group">
          {rows.map((row) => {
            const full = row.signups.length >= row.maxSignups;
            ...
            return (
              ...
              >
                ...
                >
                  <MobileSectionLabel>Assignment</MobileSectionLabel>
                  <div className="space-y-2 md:space-y-1.5">
                    ...
                    <p className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 text-[0.9375rem] leading-snug sm:text-base">
                      <span className="font-semibold tracking-tight text-foreground">{row.label}</span>
                      <span
                    ...
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                      {row.signups.length} of {row.maxSignups} spots filled
                    </p>
                  </div>
                </th>
                ...
                <td className="block border-t border-border/40 bg-muted/[0.07] px-5 pb-6 pt-5 md:table-cell md:border-0 md:bg-transparent md:px-4 md:py-4 md:pb-4 md:pt-4 lg:px-5">
                  ...
                  {!signupsEnabled ? (
                    ...
                    <div className="space-y-3 rounded-2xl border border-dashed border-border/55 bg-background/60 px-4 py-4 text-base text-muted-foreground sm:text-sm md:bg-muted/20">
                      <p>All {row.maxSignups} spots are filled.</p>
                      <Button
                        type="button"
                        disabled
                        className="w-full touch-manipulation py-3.5 sm:py-2.5 md:w-auto md:py-2.5"
                    ...
                    >
                      <input type="hidden" name="signup_key" value={row.key} />
                      <label className="block text-base font-medium text-foreground sm:text-sm md:text-sm">
                        <span className="sr-only">Your name for {row.title} on {row.label}</span>
                        <input
                          name="name"
                          type="text"
                          required
